fn main() -> std::process::ExitCode {
    ...
    let Some(path) = std::env::args().nth(1) else {
        log::error!(
            "Usage: {} <path>",
            std::env::args().nth(0).unwrap_or("command".to_string())
        );
        return std::process::ExitCode::FAILURE;
    };

    let path = std::path::Path::new(&path);
    let mut watcher = watcher::WatcherImpl::new();
    let rx = match watcher.watch(path) {
        Ok(rx) => rx,
        Err(watcher::Error::NotFound(err)) => {
            log::warn!("Error: {err:?}");
            return std::process::ExitCode::FAILURE;
        }
        Err(watcher::Error::Internal(err)) => {
            log::error!("Error: {err:?}");
            return std::process::ExitCode::FAILURE;
        }
    };
    let rx = watcher::relay_latest(rx, std::time::Duration::from_millis(100));
...
mod watcher {
    ...
    pub trait Watcher {
        fn watch(&mut self, path: &std::path::Path)
            -> Result<std::sync::mpsc::Receiver<()>, Error>;
    }
